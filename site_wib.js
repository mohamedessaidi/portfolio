...
(function () {
	...
	function initMobileMenu() {
		...
		document.addEventListener('click', function (e) {
			if (!header.contains(e.target) && window.getComputedStyle(nav).display === 'block') {
				setNav(false);
			}
		});
	...
	function initRevealOnScroll() {
		...
		const els = document.querySelectorAll('.card, .hero, .work h2, .about-me, .social-media, .social_media, .site-footer');
